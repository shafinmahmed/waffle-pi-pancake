...
def ex_callback_0(data):
    if (data == 0):     # condition to trigger move base goals to marker location
        global newFlag
        newFlag = True

def ex_callback_1(data):
    if (data == 1):     # condition to trigger move base goals to marker location
        global newFlag
        newFlag = True
...
def calc_go_to_path_points():
    ...
    publisher2 = rospy.Publisher('go_to_maker', Int32, queue_size=50)
